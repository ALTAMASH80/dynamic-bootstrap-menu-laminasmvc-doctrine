...
class PickleTree {
    ...
    staticEvents() {
        ...
        if (this.config.drag) {
            ...
            document.getElementById('div_pickletree').addEventListener("drag", e => {
                ...
                //show hidden tooltip
                this.div_detail.style.display = '';
                //set cordinates as mouse side
                this.div_detail.style.top = (e.clientY) + 'px';
                this.div_detail.style.left = (e.clientX + 40) + 'px';
                //set title to inside
                this.div_detail.innerHTML = '<span>' + e.target.getAttribute('drag-title') + '</span>';

            });
            //drag end
            document.getElementById('div_pickletree').addEventListener("dragend", e => {
                //make tootlip invisable again
                this.div_detail.style.display = 'none';
                //clear old targets
                this.clearDebris();
                let node = this.nodeList[(e.target.id.split('node_')[1])];
                //set old parent for cleaning
                node.old_parent = node.parent;
                if (this.drag_target === (e.target.id.split('node_')[1])) {
            ...
            document.getElementById('div_pickletree').addEventListener("dragenter", (e) => {
                ...
                if (e.target.classList.contains("drop_target")) {
                    ...
                    this.drag_target = (e.target.id.split('node_')[1]);
                }
            });
        }
    }
    ...
    getAllNodes(){return this.nodeList;}
    ...
    drawNode(node) {
        ...
        if (node.elements.length > 0) {
            ...
            let a_item = document.createElement('span');
            for (let i = 0; i < node.elements.length; i++) {
                let i_item = document.createElement('i');
                i_item.className = node.elements[i].icon;
                i_item.setAttribute('id', node.elements[i].id);
                a_item.appendChild(i_item);
            }

            a_item.id = 'a_me_' + node.id;
            a_item.className = 'fa-hover menuIcon';
            //icon added to div
            div_item.appendChild(a_item);
        }
    ...
    saveNodeChanges(node, element, action){
        if(action === 'move-down'){
            node.sibling = 'movedown_' + element.previousElementSibling.id;
    ...
    drawData() {
        ...
        if (this.config.drag) {
            this.div_detail = document.createElement('div');
            this.div_detail.id = 'div_detail';
            this.div_detail.style.position = 'absolute';
            this.div_detail.style.display = 'none';
            this.div_detail.innerHTML = '<span>No Element</span>';
            document.querySelector('body').appendChild(this.div_detail);
        }
